...
public class removeManager {
	...
	public static void main(Player owner, Player target) {
		...
		if(!guildAPI.getPlayerRank(target.getName()).equalsIgnoreCase(Core.managerName)) {
			...
			owner.sendMessage(color.add(Core.getlang("manager.targetIsNotManger").replace("%player%", target.getName())));
			return;
	...
	public static void succes(Player owner, Player target) {
		...
		File folder = new File(guildsPath+"/"+data.guildData.convertGuildNameToGuildFileName.get(guildAPI.getPlayerGuildName(owner.getName())));
		YamlConfiguration guild = YamlConfiguration.loadConfiguration(folder);
		
		guild.set("members."+target.getName()+".rank", Core.memberName);
		
		owner.sendMessage(color.add(Core.getlang("manager.remove.succes.P").replace("%player%", target.getName())));
		target.sendMessage(color.add(Core.getlang("manager.remove.succes.T").replace("%player%", owner.getName())));
