...
public class disband {
	...
	public static HashMap<Player, Integer> DisbandTimeouts = new HashMap<Player, Integer>();
	public static HashMap<Player, Integer> DisbandTimeoutsGUI = new HashMap<Player, Integer>();
	...
	public static void CHATform(Player p) {
		...
		DisbandTimeouts.put(p, Core.getInstance().getConfig().getInt("disband.expiresTime.chat"));

		for (String str : langFile.get().getStringList("disband.message")) {
			str = str.replace("%s%", String.valueOf(DisbandTimeouts.get(p)));
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				if (DisbandTimeouts.get(p) > 0 && requestDisband.contains(p.getUniqueId()))
					DisbandTimeouts.put(p, DisbandTimeouts.get(p) - 1);
	...
	public static void succes(Player p) {
		...
		for (String player : playerGuildData.getGuildName.keySet()) {
			
			if(playerGuildData.getGuildCustomName.get(player)!=null)
				if(playerGuildData.getGuildName.get(player).equals(guildName)) 
					playerGuildData.loadPlayerData(player);
