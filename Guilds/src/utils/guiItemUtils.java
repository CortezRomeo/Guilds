...
public class guiItemUtils {	
	...
	public static ItemStack getItem(String type, String value, Short id, String name, String target, List<String> lore,
			String replace, File folderGuild) {
		...
		if (type.equalsIgnoreCase("customhead")) {
			...
			if (replace.equals("guildStats")) {
				name = name.replace("%guildName%",
						data.guildData.convertGuildFileNameToGuildName.get(folderGuild.getName()));
			}
			...
			for (String string : createLore) {
				...
				string = string.replace("%totalguild%", String.valueOf(guildAPI.getTotalGuild()));

				if (replace.equals("guildStats")) {
					YamlConfiguration cfg = YamlConfiguration.loadConfiguration(folderGuild);
					String guildName = cfg.getString("guildName");
					String dateString = utils.dateUtils
							.main(Integer.valueOf(guildAPI.getGuildFoundedTime(guildName)));

					string = string.replace("%chuSoHuu%", guildAPI.getChuGuild(guildName));
					string = string.replace("%totalMembers%",
							String.valueOf(guildAPI.getGuildTotalMember(guildName)));
					string = string.replace("%board%", guildAPI.getGuildBoard(guildName));
					string = string.replace("%customname%", guildAPI.getGuildCustomName(guildName));
					string = string.replace("%foundedTime%", dateString);

				}
				if (replace.equals("memberStats")) {

					String dateString = utils.dateUtils.main(guildAPI.getPlayerJoinDate(target));
					string = string.replace("%rank%", (guildAPI.getPlayerRank(target)));
				...
				if (replace.equals("timeout")) {
					...
					string = string.replace("%seconds%", String.valueOf(disbandGUI.timeout.get(target) + 1));
